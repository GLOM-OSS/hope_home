...
const languageReducer: Reducer<Language, Action> = (
  ...
  switch (action.type) {
    case 'USE_ENGLISH': {
      localStorage.setItem('hope_hope_active_language', 'En');
      return { ...state, activeLanguage: 'En' };
    }
    case 'USE_FRENCH': {
      localStorage.setItem('hope_hope_active_language', 'Fr');
...
function LanguageContextProvider({
  ...
  const initialState: Language = {
    activeLanguage:
      (localStorage.getItem('hope_hope_active_language') as LanguageType) || 'Fr',
